package com.martiply.android.view.fsv;
...
import static com.mypopsy.widget.internal.RoundRectDrawableWithShadow.*;

/**
 * Created by desktop on 2016/10/18.
 * This is FloatingSearchView fsv from https://github.com/renaudcerrato/FloatingSearchView
 * Had to import because elevation, color, etc are harcoded
 */

public class MyFloatingSearchView extends RelativeLayout {
...
//    private static final int DEFAULT_MAX_ELEVATION = 2;
    ...
    private RoundRectDrawableWithShadow mSearchBackground;
    private SuggestionItemDecorator mCardDecorator;
    ...
    public MyFloatingSearchView(Context context, AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    private void applyXmlAttributes(AttributeSet attrs, @AttrRes int defStyleAttr, @StyleRes int defStyleRes) {
        final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.MyFloatingSearchView, defStyleAttr, defStyleRes);
...
//                ViewUtils.dpToPx(DEFAULT_MAX_ELEVATION));
        ...
        mSearchBackground = new RoundRectDrawableWithShadow(
        DEFAULT_CONTENT_COLOR, ViewUtils.dpToPx(DEFAULT_RADIUS),
        a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_fsv_my_elevation, ViewUtils.dpToPx(DEFAULT_ELEVATION)),
        a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_fsv_my_elevation, ViewUtils.dpToPx(DEFAULT_MAX_ELEVATION)));
        ...
        int searchBarWidth = a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_fsv_my_searchBarWidth, mSearchContainer.getLayoutParams().width);
        mSearchContainer.getLayoutParams().width = searchBarWidth;
        suggestionsContainer.getLayoutParams().width = searchBarWidth;

        // Divider
        mDivider.setBackground(a.getDrawable(R.styleable.MyFloatingSearchView_android_divider));
        int dividerHeight = a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_android_dividerHeight, -1);

        MarginLayoutParams dividerLP = (MarginLayoutParams) mDivider.getLayoutParams();

        if(mDivider.getBackground() != null && dividerHeight != -1) dividerLP.height = dividerHeight;

        float maxShadowSize = mSearchBackground.getMaxShadowSize();
        float cornerRadius = mSearchBackground.getCornerRadius();
        int horizontalPadding = (int) (RoundRectDrawableWithShadow.calculateHorizontalPadding(maxShadowSize, cornerRadius, false) + .5f);
        ...
        int contentInsetStart = a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_contentInsetStart, MarginLayoutParamsCompat.getMarginStart(searchParams));
        int contentInsetEnd = a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_contentInsetEnd, MarginLayoutParamsCompat.getMarginEnd(searchParams));
        ...
        // anything else
        setLogo(a.getDrawable(R.styleable.MyFloatingSearchView_logo));
        setContentBackgroundColor(a.getColor(R.styleable.MyFloatingSearchView_fsv_my_contentBackgroundColor, DEFAULT_CONTENT_COLOR));
        setRadius(a.getDimensionPixelSize(R.styleable.MyFloatingSearchView_fsv_my_cornerRadius, ViewUtils.dpToPx(DEFAULT_RADIUS)));
        inflateMenu(a.getResourceId(R.styleable.MyFloatingSearchView_fsv_my_menu, 0));
        setPopupTheme(a.getResourceId(R.styleable.MyFloatingSearchView_popupTheme, 0));
        setHint(a.getString(R.styleable.MyFloatingSearchView_android_hint));
        setIcon(a.getDrawable(R.styleable.MyFloatingSearchView_fsv_my_icon));
    ...
    public void inflateMenu(@MenuRes int menuRes) {
        ...
        try (XmlResourceParser parser = getResources().getLayout(menuRes)) {
...
//        }
        ...
        if(icon != null) {
            @SuppressLint("ObjectAnimatorBinding") ObjectAnimator iconAnim = ObjectAnimator.ofFloat(icon, "progress", enter ? 1 : 0);
